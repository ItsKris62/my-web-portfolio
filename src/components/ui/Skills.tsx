...
const RadarChart = ({ skills }: { skills: Skill[] }) => {
  ...
  const isInView: boolean = useInView(chartRef, { once: true });
      ...
      <svg width="300" height="300" className="mx-auto">
        ...
        <motion.polygon
          points={skills.map((skill, i) => {
            const angle = i * angleStep - Math.PI / 2;
            const distance = ((skill.level || 0) / 100) * radius;
...
const HexagonGrid = ({ skills }: { skills: Skill[] }) => {
  return (
...
const SkillBars = ({ skills }: { skills: Skill[] }) => {
  return (
      ...
      {skills.map((skill, i) => (
        ...
        >
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-xs text-[#278783]/70 font-['Inter']">{skill.level || 0}%</span>
          </div>
          <div className="h-2 bg-[#FFEBD0]/20 rounded-full overflow-hidden">
            <motion.div
              className={`h-full bg-gradient-to-r ${categoryColors[skill.category || 'networking'] || "from-[#278783] to-[#4AB8B3]"} rounded-full`}
              initial={{ width: 0 }}
...
const FloatingCards = ({ skills }: { skills: Skill[] }) => {
  return (
...
const Skills = () => {
  ...
  if (!isMounted) {
    return (
      <section className="py-20 bg-[#FFEBD0] relative overflow-hidden">
